...
TOKEN = "8876754975:AAHkEWGM0CAdySXT9yMbV7y_0QiIt-EQ7yc" 

# ── Join requirements ─────────────────────────────────────────────────────────
join_channel_id = -1003961440425       # Replace with your channel ID
join_chat_id    = -1003961440425       # Replace with your group ID

CHANNEL_LINK = "https://t.me/blackshopmart"
GROUP_LINK   = "https://t.me/+blackshopmart"
...
async def _dork_bg(uid: int, query: str, proxy_str: str | None, wait_msg):
    ...
    caption = (
        ...
        f"{pe(E['star'])} {bold('@BSM_CheckerBot')}"
    )
...
async def cmd_sh(message: types.Message):
    ...
    if not auth.has_premium_access(user_id, message.chat.id):
        await message.reply(
            ...
            f"\n\n{pe(E['bolt'])} {bold('Contact admin or redeem a key')}"             f"\n{pe(E['next'])} t.me/Blackshopmart_owner "
            f"\n{pe(E['next'])} /redeem {bold('Black-xxxxx')}"
        )
        return
...
async def cmd_redeem(message: types.Message):
    ...
    if len(args) < 2 or not args[1].strip():
        await message.reply(
            f"{pe(E['warn'])} {bold('Usage:')} /redeem {bold('Black-xxxxx')}"
        )
        return
...
FEEDBACK_GROUP_ID = -1004393203720
...
async def cb_feedback_accept(callback: types.CallbackQuery):
    ...
    group_text = (
        ...
        f"{_fb_e()} {bold('Bot:')} @BSM_CheckerBot\n"
...
async def main():
    me = await bot.get_me()
    log.info(f"@BSM_CheckerBot is running...")
